# Hi, I'm Tejas G 👋

### Aspiring Software Engineer | AI & ML Enthusiast | Python Developer

I am an aspiring **Software Engineer** with a strong interest in **Artificial Intelligence, Machine Learning, and Software Development**. I enjoy building practical applications, solving problems, and continuously improving my engineering skills through hands-on projects and learning.

## 👨‍💻 About Me

- 🎓 Computer Science student specializing in **Artificial Intelligence & Machine Learning**
- 💻 Interested in **Software Development, Python, AI, and Machine Learning**
- 🧠 Building strong foundations in **Data Structures, Algorithms, and Object-Oriented Programming**
- 🚀 Focused on developing practical projects and writing clean, maintainable code
- 📚 Continuously learning new technologies and software engineering practices
- 🤝 Open to collaboration, learning opportunities, and meaningful projects

## 🛠️ Technical Skills

**Languages**

`Python` `Java` `C`

**Core Concepts**

`Data Structures & Algorithms` · `Object-Oriented Programming` · `DBMS` · `Operating Systems` · `Artificial Intelligence` · `Machine Learning`

**Tools & Technologies**

`Git` · `GitHub` · `VS Code` · `Jupyter Notebook`

## 🎯 Professional Focus

My goal is to grow into a capable software engineer who can design, build, and improve reliable software systems. I am particularly interested in combining strong software engineering fundamentals with Artificial Intelligence and Machine Learning to solve real-world problems.

## 📌 What You'll Find Here

- 💻 Programming and software development projects
- 🤖 AI and Machine Learning experiments
- 🧩 Data Structures and Algorithms practice
- 📚 Academic and personal learning projects
- 🚀 Projects built while exploring new technologies

## 🌱 Currently Learning

I am continuously strengthening my knowledge of **Python, software development, problem-solving, AI, and Machine Learning**, while working toward building increasingly practical and impactful projects.

## 🌐 Connect With Me

<p align="left">
  <a href="mailto:tejasguru53@gmail.com">
    <img src="https://img.shields.io/badge/Email-tejasguru53%40gmail.com-red?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://www.linkedin.com/in/tejas-g-93a254272/">
    <img src="https://img.shields.io/badge/LinkedIn-Profile-blue?style=for-the-badge&logo=linkedin" alt="LinkedIn" />
  </a>
  <a href="https://instagram.com/tejaz.g">
    <img src="https://img.shields.io/badge/Instagram-@tejas.g-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram" />
  </a>
</p>

---

> **"Keep learning. Keep building. Keep improving."**

⭐ Feel free to explore my repositories and follow my journey as I continue building and learning.
